using System.Runtime.CompilerServices;

[assembly: InternalsVisibleTo("Sharpen.Engine.Tests.Unit, PublicKey=")]
// ReSharper disable CheckNamespace
internal static partial class AssemblyDescription
{
    public const string Title = "Sharpen Engine Abstractions";

    public const string Description = "Common abstractions implemented by different Sharpen Engines that target different versions of Roslyn.";

    public const string InformationalVersion = Version;
}
// ReSharper restore CheckNamespace